...
class ReadMe:
    ...
    def get_lines_for_election_result(election):
        ...
        for result in election.results:
            ...
            table_lines.append(
                f'| {result.row_num} | {result.pds_short} | {result.electorate_name} | {winning_single_result.party_emoji_and_code} | `{winning_single_result.party_symbol}` | {winning_single_result.candidate} | {winning_single_result.votes:,} |'
            )
        table_lines.append('')
        lines += table_lines
        lines.extend(
